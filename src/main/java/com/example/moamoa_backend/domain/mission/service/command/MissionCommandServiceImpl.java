...
public class MissionCommandServiceImpl implements MissionCommandService {
	...
	@Transactional
	@Override
	public MissionResponseDto.StatusResult updateMissionStatus(Long memberId, Long missionId,
	...
	@Transactional
	@Override
	public MissionResponseDto.SubmitResult submitMissionAnswer(Long memberId, Long missionId,
		...
		//기록 조회 및 첫 시도 판별
		MemberMission memberMission = memberMissionRepository.findByMemberIdAndMissionId(member.getId(),
				mission.getId())
			.orElse(null);

		//첫 시도 여부 판별
		//기록이 아예 없거나, 기록은 있는데 (NONE 상태 && 시도횟수 1)
		boolean isFirstAttempt = (memberMission == null) || (memberMission.getMissionStatus() == MissionStatus.NONE
			&& memberMission.getAttemptCount() <= 1);
		...
		if (!isAllCorrect) {
			...
			if (isFirstAttempt) {
				...
				updateWalletAndSaveHistory(wallet, mission, TransactionType.MISSION, earnedScore,
					mission.getTitle() + desc);
		...
		if (isFirstAttempt) {
			...
			updateWalletAndSaveHistory(wallet, mission, TransactionType.MISSION_COMPLETE, finalMissionReward,
				mission.getTitle() + "성공");
			walletHistoryRepository.flush();
		} else {
	...
	// [신규 생성] 기록이 아예 없을 때
	private MissionResponseDto.StatusResult createInitialMemberMission(Long memberId, Mission mission,
